...
| ドキュメント | 中身 |
|---|---|
| [`USAGE.md`](./USAGE.md) | 当日の操作（画面写真つき）。部屋を用意して見守る人向け |
...
読む人で2つに分かれる。子どもが見る画面はひらがな寄り、大人が見る画面は漢字混じりで書いている。

| URL | 読む人 | 中身 |
|---|---|---|
| `/` | 子ども | 部屋コードを入れる |
| `/r/<22文字のID>` | 子ども | 入室（なまえと合言葉）とチャット |
| `/help` | 子ども | [使い方](#使い方のページ) |
| `/admin` | 大人 | [管理画面](#管理画面)（要ログイン） |
| `/admin/help` | 大人 | [管理画面の使い方](#使い方のページ)（ログイン不要） |

ルーティングは `web/src/app/App.tsx` の1か所。本番はどのURLでも BFF が `index.html` を返すので、
画面を増やしてもWebサーバー側に足すものは無い。
...
`/admin`。大人が見る画面なので、文言はひらがな寄りにしていない。操作の手順はヘッダーの
「使い方」（`/admin/help`）にまとめてある。
...
説明は読む人で2枚に分けてある。文体も中身も相手が違うので、1枚にまとめていない。

| | `/help` | `/admin/help` |
|---|---|---|
| 読む人 | 子ども | 大人（管理者） |
| 入口 | トップページの「つかいかたを 見る」 | 管理画面ヘッダーの「使い方」 |
| 中身 | 部屋の入り方、はなす、AIに きく、ファイル、でる、こまったとき、やくそく | 全体の流れ、権限、部屋を作る、渡し方、AIのモード、AIの中身、見守る・片づける、上限と注意 |
| ログイン | 不要 | 不要（操作の説明だけで、部屋の中身は出さない） |

同じ内容を、リポジトリだけで読める形にしたのが [`USAGE.md`](./USAGE.md)。画面写真は
`/admin/help` と同じものを使っている。画面を直したら、こちらの写真も差し替える。

`/help` は部屋に入る前からも開くので、そこからは部屋のモードが分からない。会話モードと意見モードの
両方のボタンを並べて、「じぶんの へやに ある ボタンを つかってね」と書いている。
...
- [x] フェーズ13 使い方のページ（子ども向けと管理者向けの2枚）
