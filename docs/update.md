...
サーバーは SIGTERM を受けてから次の順で畳むので、生成中の意見が消えることはない。
...
2人で発言する、AIに意見を聞く、画像を1枚送る、あたりまで通す（[`development.md` の4章](./development.md#4-通しで動かしてみる)）。
...
[`deploy.md` の4章](./deploy.md#4-動いているか確かめる)と同じ通し（2人で発言・AIに意見を聞く・画像1枚）をやっておく。
